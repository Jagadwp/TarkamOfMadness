...
function connected(socket){
    ...
    if (clientNo % 2 === 1){  
        ...
        roomData[roomNo] = {}
        roomData[roomNo].p1ID = socket.id
        roomData[roomNo].p1X = 115
        roomData[roomNo].p1Y = 280
        roomData[roomNo].p1score = 0
        roomData[roomNo].p1point = 0
    }
    else if (clientNo % 2 === 0){
        //creating player 2 and football
        serverBalls[socket.id] = new Capsule(560, 280, 490, 280, 25, 10);
        serverBalls[socket.id].no = 2;
        serverBalls[socket.id].layer = roomNo;
        roomData[roomNo].p2ID = socket.id
        roomData[roomNo].p2X = 525
        roomData[roomNo].p2Y = 280
        roomData[roomNo].p2score = 0
        roomData[roomNo].p2point = 0
        football[roomNo] = new Ball(320, 280, 20, 6);
        football[roomNo].layer = roomNo;
        roomData[roomNo].ballX = 320
        roomData[roomNo].ballY = 280
        console.log("Emitting: ",roomData[roomNo])
        io.emit('gameSetup', roomData[roomNo]);
    }
    ...
    console.log(roomData[serverBalls[socket.id].layer]);
