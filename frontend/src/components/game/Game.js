...
const Game = () => {
    ...
    if (ctx) {
      ...
      class Capsule extends Body {
        constructor(x1, y1, x2, y2, r, m) {
          ...
          this.inertia =
            (this.m * ((2 * this.comp[0].width) ** 2 + (this.comp[0].length + 2 * this.comp[0].width) ** 2)) / 12;
          if (this.m === 0) {
            this.inv_inertia = 0;
          } else {
        ...
        BODIES.forEach((b, index) => {
          for (let bodyPair = index + 1; bodyPair < BODIES.length; bodyPair++) {
            if (
              ...
              COLLISIONS.push(
                new CollData(BODIES[index], BODIES[bodyPair], bestSat.axis, bestSat.pen, bestSat.vertex)
              );
            }
          }
        });
      ...
      socket.on("positionUpdate", (data) => {
        ...
        if (data.ballX > 340) {
          football.color = "blue";
        } else if (data.ballX < 300) {
          football.color = "green";
        } else {
          football.color = "red";
        }
      });
  ...
  const quitGame = () => {
    if (clientBalls[p1ID] && clientBalls[p2ID] && (clientBalls[p1ID].point > 1 || clientBalls[p2ID].point > 1)) {
      socket.emit("inGameMsg", {
    ...
    <div>
      <div className="InGameDiv">
        ...
        <div className="GameFooter">
          ...
          <Button className="QuitButton" onClick={quitGame}>
            {" "}
            QUIT GAME{" "}
          </Button>{" "}
        </div>{" "}
